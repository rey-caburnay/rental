...
/* package */
class NPPSInvocationHandler implements InvocationHandler {
    /**
     * Standard PreparedStatement used to implement the PreparedStatement interface.
     */
    private PreparedStatement statement;

    /**
     * Maps each parameter name to a list of integers: the parameter indices. This enables the same
     * value to be used in multiple locations in the statement.
     */
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        boolean isSetter =
                args != null && args.length > 1 && "set".equals(method.getName().substring(0, 3));

        if (isSetter && args[0] instanceof String) {
            Class[] paramTypes = method.getParameterTypes();
            paramTypes[0] = int.class;
            Method statementMethod = statement.getClass().getMethod(method.getName(), paramTypes);
            ...
            throw new RuntimeException(
                    "Cannot set value by index on a NamedParameterPreparedStatement");
        } else {
