...
import static com.shinn.configuration.ServerSecurityConfig.REMEMBER_ME_KEY;
...
import com.shinn.dao.repos.TokenDao;
...
public class RememberMeServices extends
    ...
    public RememberMeServices(Environment env, org.springframework.security.core.userdetails.UserDetailsService userDetailsService) {
        ...
        super.setParameter(DEFAULT_PARAMETER);
        this.setParameter(DEFAULT_PARAMETER);
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (rememberMeCookie != null && rememberMeCookie.length() != 0) {
            try {
                ...
                Token token = getPersistentToken(cookieTokens);
                tokenDao.delete(token.getSeries());
                tokenDao.commit();
    ...
    private Token getPersistentToken(String[] cookieTokens) {
        ...
        Token token = null;
        try {
            token = tokenDao.findBySeries(presentedSeries);
        ...
        if (DateUtil.addDays(token.getDate(), TOKEN_VALIDITY_DAYS).before(new Date())) {
            tokenDao.delete(token.getSeries());
            tokenDao.commit();
