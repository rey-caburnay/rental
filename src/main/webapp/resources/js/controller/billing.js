  ...
  var BillingController = function($scope, $filter, transactionService,
    ...
    vm.computeBilling = function() {
      if (vm.rooms && vm.rooms.length > 0) {
        ...
        for (var i = 0; i < vm.rooms.length; i++) {
          ...
          vm.bill.subTotal1 = vm.bill.transmissionCharge + vm.bill.generationCharge + vm.bill.systemLossCharge;
          ...
          vm.bill.subTotal3 = vm.bill.subsidyOnLifelineCharge + vm.bill.seniorCitizenSubsidyCharge + vm.bill.surcharge;
          
          // government charges
          vm.bill.tax.localFranchise = electricParam.localFranchiseTax * (vm.bill.subTotal1 + vm.bill.subTotal2 + vm.bill.subTotal3);
          vm.bill.tax.generation = room.diffReading * electricParam.generationTax;
          vm.bill.tax.transmission = room.diffReading * electricParam.transmissionCharge;
          vm.bill.tax.systemLoss = room.diffReading * electricParam.systemLossTax;
          vm.bill.tax.distribution = electricParam.distributionTax * vm.bill.subTotal2;
          vm.bill.tax.others = electricParam.othersTax * vm.bill.subTotal3;
          vm.bill.tax.missionaryElectrification = room.diffReading * electricParam.missionaryElectrification;
          vm.bill.tax.environmental = room.diffReading * electricParam.environmentalCharge;
          vm.bill.tax.npc = room.diffReading * electricParam.npc;
          vm.bill.tax.fitAllRenewable = room.diffReading * electricParam.fitAllRenewable;
          vm.bill.subTotal4 = vm.bill.tax.localFranchise + vm.bill.tax.generation +
            vm.bill.tax.systemLoss + vm.bill.tax.distribution + vm.bill.tax.others
            vm.bill.tax.environmental + vm.bill.tax.npc + vm.bill.tax.fitAllRenewable;
          vm.bill.grossAmount = vm.bill.subTotal1 + vm.bill.subTotal2 + vm.bill.subTotal3 + vm.bill.subTotal4 +
          room.overdue;
          vm.bill.currentAmount = vm.bill.subTotal1 + vm.bill.subTotal2 + vm.bill.subTotal3 + vm.bill.subTotal4;
    ...
    $scope.$watch(function(scope) {
            ...
            function(newValue, oldValue) {
              ...
              if (vm.rooms && vm.rooms.length > 0) {
                for (var i = 0; i < vm.rooms.length; i++) {
                  if (vm.rooms[i].currentReading < 1 || vm.rooms[i].currentReading == "") {
                    vm.errors.currentReading = true;
                  }
                  vm.rooms[i].diffReading = (vm.rooms[i].currentReading || 0) - (vm.rooms[i].previousReading || 0);
